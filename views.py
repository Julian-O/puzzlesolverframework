...
class Controller(object):
    ...
    def register(self, surface, callback, cookie):
        ...
        bounding_box = (
            top_left_corner[0],
            top_left_corner[1],
            top_left_corner[0] + size[0],
            top_left_corner[1] + size[1],
        )
    ...
    def _find_and_notify_subscriber(self, button, offset):
        for (bounding_box, callback_key) in self.bounding_boxes.items():
            if (
                ...
                and bounding_box[1] < offset[1]
                and bounding_box[2] > offset[0]
                and bounding_box[3] > offset[1]
            ):
                ...
                callback(
                    button,
                    (offset[0] - bounding_box[0], offset[1] - bounding_box[1]),
                    cookie,
                )
...
try:
    ...
    class PyGameController(Controller):
        ...
        def default_solver(
            self, work_cycle_len=500, sleep_cycle_len=250, keyboard_callback=None
        ):
